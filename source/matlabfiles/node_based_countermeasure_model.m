...
%degree=load('../../data/ba_degree_info.txt')';%各ノードの次数
tau=0.001;
deltaw=1;
omega=0.5;
kappa=0.1;
t0 = 0; tf = 1000; tspan = [t0,tf]; %tの範囲の指定
...
xlabel('Time','FontSize',15); ylabel('Population Fraction','FontSize',15); %軸の指定
legend('Location','northoutside')%凡例の位置をグラフ外に設定
legend('Orientation','horizontal')
legend('NumColumns',5)
legend('Box','off')
legend('Susceptible','Infected','Recovered','CM','FontSize',9) %凡例の追加と文字の大きさの指定
